...
Representations in Global Land Surface Models: CLM-PFLOTRAN 1.0}
...
surface models; our goal is to facilitate testing of alternative models and incorporation of
...
reactions in the open-source PFLOTRAN code, coupled with the Community Land Model
...
false convergence, an accurate solution can be achieved with about 50\% more
computing time than CLM in point mode site simulations using either the scaling
or clipping methods. The log transformation method takes 60 $\sim$ 100\% more computing time
than CLM. The computing time increases slightly for clipping and scaling; it increases
...
nitrogen concentrations. The frequent occurrence of very low concentrations (e.g.
below nanomolar) can increase the computing time for clipping or scaling by
about 20\%; computing time can be doubled for log transformation. Caution needs to be taken in
choosing the appropriate scaling factor  
because a small value caused by a negative update to a
...
models \citep{Seneviratne2010}, current LSMs such as the Community Land Model
...
modeling (RTM) code, was used to develop multiphase mechanistic carbon and
nitrogen models with many speciation and microbial reactions
\citep{Maggi2008,Gu2010,Riley2014}, but it has not been coupled to an LSM. PHREEQC
...
mineral nitrogen, phosphorus) among plants and microbes. In CLM,  plant
...
the threshold, and half saturation is at or below nanomolar ($10^{-9}$
...
\citep{Hungate1975}] for methanogens to grow
\citep{Jarrell1985}. 
...
As LSMs need to run under various conditions at the global scale for simulation
...
ordinary differential equations for the kinetic reactions and mass action
...
relatively small in this study, we solve them directly via LU (lower upper) factorization.
...
provides an option to calculate derivatives numerically via finite-differencing, we use analytical expressions for efficiency and accuracy. 
...
response functions is coded in PFLOTRAN. Most of the biogeochemical reactions
...
traditionally hardcoded in CLM. Consequently, any change of the pools,
...
\end{equation}
Here \chem{CN_u} is the upstream pool with 1:u as the CN ratio in mole; []
denotes concentration; and $k_\text{d}$, $f_\text{T}$, and $f_\text{w}$ are the
rate coefficient and temperature and moisture response functions, respectively. When \chem{CN_u}
...
\label{eq:plantnrate}
\end{equation}
In this equation $R_p$, $R_a$, and $R_n$ are the plant uptake rates for nitrogen, ammonium
...
CLM uses a demand-based competition approach (Appendix A, Section
...
$k_\text{m}$ values was reported for ammonium, nitrate, and organic nitrogen
...
considered to be zero \cite[e.g.,][]{Hogh1997}. The detection limits are usually at the micromolar
level, while up to the nanomolar level was reported \citep{Nollet2013}. In Ecosys,
...
\label{sec:test1}
It was observed that plants can decrease nitrogen concentration to below the
...
converge to the nonphysical negative semianalytical solution (spreadsheet case3).
This can be avoided by using clipping, scaling, or log transformation (spreadsheet
case4, case6, case8). 

Even though clipping avoids convergence to the negative solution, the ammonium
consumption is clipped, but the \chem{PlantA} production is not clipped (spreadsheet case5),
...
\unit{mol\,m^{-3}} (e.g., soil organic matter pool 4 SOM4 in organic layer), the relative update $snorm_{rel}$ =
...
result in mass balance errors for clipping and scaling.
...
\label{eq:ex2}
\end{equation}
A semianalytical solution similar to Eq. (\ref{eq:monodsemi}) can be derived for Eq. (\ref{eq:ex2}). With $J_{at} =
...
negative even when the ammonium concentration is not very low. 
...
The scaling factor ($\lambda$) is not only a function of the update; it is a function
...
is not too small, say $10^{-6}$ \unit{M}, the solution converges to the semianalytical
solution in six iterations. When [\chem{NO_3^-}]$_0$ is decreased to 10$^{-9}$
...
not to the positive semianalytical solution. The ammonium uptake and nitrification
...
iterations continue, with decreasing nitrate concentration, $\lambda$, and
...
produced even for products during a Newton-Raphson iteration; and 2) when a negative
...
denitrification added to Test 2 are available in Appendix \ref{sec:eqtest3}. In addition
...
result, a negative update for both ammonium and nitrate contributes to
...
than \chem{PlantA} as
the former is influenced by more rates and derivatives. We add plant nitrate
...
rebounds after Lit1 is depleted. For $k_\text{m}$ values of 10$^{-6}$, 10$^{-9}$, and
10$^{-12}$ M, \chem{Lit1} and \chem{SOM1} dynamics are similar except for slight
differences in the early transit periods, but the ammonium values are decreased
to $\sim$1$0^{-8}$, 10$^{-11}$, and 10$^{-14}$ \chem{M}, respectively. Smaller
$k_\text{m}$ values result in lower ammonium concentrations, which have implications
for the clipping, scaling, and log transformation methods as discussed in Tests
1--3. 
...
$f_{pi} = ({R_a + R_n})/{R_p}$ as implemented in Eqs. (\ref{eq:plantarate}) and (\ref{eq:plantnrate}).
...
Various levels of preferences of ammonium over nitrate uptake were observed for plants
...
subsurface chemistry (decomposition, nitrification, denitrification, and plant
...
leaching are ignored by setting 1) decomp\_depth\_efolding to 10$^6$ m, and
o\_scalar to 1, and 2) cryoturb\_diffusion, 3) som\_diffusion, and 4) sf\_no3 and
...
one-dimensional column as CLM. The simulation durations are 1000, 600, and 600
years for the Arctic, temperate, and tropical sites, respectively.
...
multiyear cycle in addition to the annual cycle (Figs. \ref{fig:brw500yl},
...
SOM (soil organic matter), ammonium, and nitrate pools for the Arctic (Fig. \ref{fig:brw500yl}),
...
differences occur during the transient periods (200$\sim$400 years for the Arctic, and 50$\sim$70
years for the temperate and tropical sites) for \chem{SOMN} (soil organic matter nitrogen), ammonium, and
...
simultaneously while the latter does so sequentially (it resolves the plant uptake and
...
\ref{fig:brw500yl}): LAI and VEGN (vegetation nitrogen) jump up at the beginning, then level off, and
drop down at the end of the growing season when LITN (litter nitrogen) jumps up due to litter fall.
...
CLM-PFLOTRAN is about 60$\sim$100\% more than that of CLM (Table
...
immobilization are inhibited at a relatively high concentration so that nitrogen
...
The results for scaling are similar to clipping: mass balance errors are
...
the ammonium concentration increases with time due to deposition, but it drops
...
every day. It worsens with further decrease of the reset concentration to
...
of the net nitrogen mineralization rate to reflect the relationship between labile carbon
...
contribute to negative updates to nitrous oxide. Similar to Tests 2$\sim$3, a negative
...
etc. Theoretically, the numerical "inhibition" of all reactions can be caused by
negative updates to very low concentrations for any species. 

The numerical errors can be decreased and eliminated by decreasing STOL.
Similar to Test 2 (SI test2.xlsx spreadsheet scale2), a small STOL can result in
...
increases about 10\% for clipping, and it doubles for the log transformation.  
...
not guarantee satisfying the residual equation, and therefore it may introduce
...
computing time is 50$\sim$100\% more than CLM4.5 for a range of half
saturation values from 10$^{-3}$ to 10$^{-9}$ and a residual concentration of
10$^{-15}$ for nitrogen. Because physical half saturation ranges from 10$^{-5}$ to
...
solutions  for current CLM soil biogeochemistry can be achieved using CLM-PFLOTRAN.
...
CLM-PFLOTRAN is under development and will be made available according to the
NGEE-Arctic Data Management Policies and Plans
(http://ngee-arctic.ornl.gov/content/ngee-arctic-data-management-policies-and-plans).
Before it becomes publicly available, please contact the corresponding authors
to obtain access to the code. 

\section{Author contribution}
G. B., B. A., R. M., J. K., and F. H. developed the CLM-PFLOTRAN framework that this
work is built upon. F.Y., G.T., G. B., and X.X. added biogeochemistry to the CLM-PFLOTRAN
...
Thanks to Kathie Tallant and Kathy Jones at ORNL for editing service. This
...
used with STOL = $10^{-8}$. \chem{N_2O-N} concentration in the y-axis in (b) is the
minimum of the ten soil layers. Numerical experiments are conducted for the
...
\chem{N_2O-N} concentration in the y-axis in (b) is the minimum of the ten soil
layers.}
...
\bottomhline
\end{tabular}
\belowtable{CLM wall time is 29.3, 17.7, and 17.1 hours for the Arctic,
...
\label{rxn:decomp}
\end{reaction}
where \chem{CN_u} and \chem{CN_d} are the upstream and downstream pools (molecular
formula, for 1 mol upstream and downstream pool, there is u and d mol N);
\chem{N} is either \chem{NH_4^+} or \chem{NO_3^-}; \textit{f} is the
respiration fraction; and \textit{n} = u $-$ (1 $-$ \textit{f})d. The rate is
\begin{equation}
\frac{d [\chem{CN_u}]}{d t} = - k_\text{d} f_\text{T} f_\text{w} [\chem{CN_u}],
\label{eq:decomprate}
\end{equation}
where $\textit{k}_\text{d}$ is the rate coefficient, and $\textit{f}_\text{T}$
and $\textit{f}_\text{w}$ are the temperature and moisture response functions.
%\begin{equation}
%f_T = Q_{10}^{(T - 25)/10}, 
%\end{equation}
%and
...
\end{equation}
where $f_\text{nm}$ is a fraction \citep{Parton1996}, and $R_\text{nm}$ is the
...
\end{equation}
where $\mathbf{c}$ is the concentration vector, $\mathbf{R}$ is the kinetic reaction rate, and $\mathbf{F}$ is the fluxes (e.g., nitrogen deposition). 
...
%\end{equation}
Here ATOL, RTOL, and STOL are the absolute residual, relative residual, and relative update tolerances.
If none of these tolerances are met in MAXIT (the maximum number of iterations before stopping the current iterations) iterations or MAXF (the maximum numb of forward function calls before stopping the current iterations) function
