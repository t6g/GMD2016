...
\Author[2,7]{Ben}{Andre}
\Author[1]{Forrest M.}{Hoffman}
\Author[5,1]{Scott L.}{Painter}
...
\affil[5]{Los Alamos National Laboratory, Los Alamos, New Mexico, United States}
\affil[6]{OFM Research, Redmond, Washington, United States}
\affil[7]{Intel Incorporation, Portland, Oregon, United States }
\affil[8]{University of Texas at El Paso, El Paso, Texas, United States}
\affil[9]{National Center for Atmospheric Research, Boulder, Colorado, United States}
...
\clearpage
\begin{abstract}
We explore a flexible and extensible approach to biogeochemistry in land
surface models to facilitate testing of alternative models and incorporation of
new understanding: coupling to a configurable subsurface reactive transport
code. A reaction network with the CLM-CN decomposition, nitrification,
denitrification, and plant uptake is used as an example. We implement the
reactions in the open source PFLOTRAN code, couple to the Community Land Model
...
implicit time stepping, the Monod substrate rate-limiting function with a
...
efficient, and robust numerical solutions, care needs to be taken to using
...
or clipping methods. Log transformation takes 60\% to 100\% more computing time
...
about 20\%, and double for log transformation. Caution needs to be taken for
the scaling method as a small scaling factor caused by a negative update to a
small concentration may diminish the update and result in false convergence
even with very tight relative update tolerance. As methane and nitrous oxide
production and consumption involve very low half saturation and threshold
concentrations, this work provides insights for addressing nonnegativity issue
and facilitates mechanistic biogeochemistry representation in earth system
...
rate coefficient, temperature and moisture response function. When \chem{CN_u}
...
rate is decreased by half. For [\chem{N}] $\gg k_\text{m}$, Eq.
...
(\chem{NO_3^-}), we need to distribute the demands between ammonium and nitrate
...
consumption is clipped but the \chem{PlantA} production is not (sheet case5),
...
small initial \chem{PlantA} concentration can cause challenge for log
transformation even though \chem{PlantA} is only a product. When it is zero,
...
a zero order ``numerical consumption'' for \chem{PlantA} or nitrate, in which
the limitation of \chem{PlantA} or nitrate availability is not explicitly
...
year for the temperate and tropical sites) for \chem{SOMN}, ammonium, and
