...
\affil[6]{OFM Research--Southwest, Santa Fe, New Mexico, United States}
...
reaction network using PFLOTRAN \citep{Lichtner2015}, couple that model to CLM, test the
...
\mathbf{c}^{k+1,p+1}$, in a Newton-Raphson iteration can result in negative
...
times the number of numerical grid cells, and $\alpha$ as a factor with default value of 0.99. RTM codes STOMP, HYDROGEOCHEM 5.0,
and TOUGH-REACT using clipping, i.e., for any $\delta \mathbf{c}^{k+1,p+1}(i)
...
$9.3\times 10^{-9}$ in the iteration (spreadsheet case5a). With a relative update
...
exponential function (spreadsheet casea, as a 64-bit real number, which corresponds
...
iteration, until stol reaches $10^{-30}$ (spreadsheet scale2). Unless the STOL is
...
be resolved in future publications. 
...
illustrate that with implicit time stepping and Newton-Raphson method, 
